...
#if 0
#define INPUTFILENAME_TS               "..\\TS\\22.ts"
#define OUTPUTFILENAME_H264            "..\\H264\\22.264"
#define OUTPUTFILENAME_NEWH264         "..\\H264\\NEW22.264"
#define OUTPUTFILENAME_MP3             "..\\MP3\\22.mp3"
#define OUTPUTFILENAME_NEWMP3          "..\\MP3\\NEW22.mp3"
#else
...
FILE * FinMp3;
FILE * FOutNEWMp3;
unsigned char * OneFrameBuf_H264;                 //一帧视频数据的BUF
unsigned char * OneFrameBuf_MP3;                  //一帧音频数据的BUF
unsigned int OneFrameLen_H264;                    //一帧视频数据的的长度
static unsigned int OneFrameLen_Mp3 ;             //一帧音频数据的的长度
static unsigned int OneFrameLen_Mp3_Fixed ;       //一帧音频数据的的长度
static unsigned int OneFrame_Mp3_Num = 0;         //读取了文件的第几帧
//CRITICAL_SECTION Cs;                              //指向线程互斥临界区的指针
//HANDLE handle_H264;                               //视频线程句柄
//HANDLE handle_Mp3;                                //音频线程句柄
pthread_t handle_H264;
pthread_t handle_Mp3;
int PrefixLen_H264 ;                              //H264一帧数据前缀的长度
unsigned long Bitrate_Mp3;                        //位率
unsigned long Sampling_Frequency_Mp3;             //采样频率
unsigned int VideoFrameCount ;                    //视频的帧号
static unsigned long long Videopts = 0;           //Videopts
static unsigned long long Videodts  = 0;          //Videodts
static unsigned long long Adudiopts = 0;
static unsigned long long Adudiodts = 0;
long VideoPtsIni;                                 //第一帧pts 初始值
double        Frame_Time;                         //一帧所用时间长度
unsigned char TSbuf[TS_PACKET_SIZE];              //一个ts包，要存入文件的
static unsigned int WritePacketNum = 0;           //已经写入文件的包个数
...
typedef struct Tag_TsPes
{
	...
	unsigned char  Es[TS_MAX_OUT_BUFF];          //一帧 原始数据
